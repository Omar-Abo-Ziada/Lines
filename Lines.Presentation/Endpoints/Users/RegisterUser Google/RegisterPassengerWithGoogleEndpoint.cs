...
public class RegisterPassengerWithGoogleEndpoint(BaseControllerParams<RegisterUserGoogleRequests> dependencyCollection) : BaseController<RegisterUserGoogleRequests, RegisterUserGoogleResponse>(dependencyCollection)
{

    [HttpGet("users/register-passenger-google")]
    public IActionResult RegisterPassengerWithGoogle()
    
    {
        // Use the exact redirect URI that matches Google Console configuration
        var redirectUrl = $"{Request.Scheme}://{Request.Host}/api/users/register-passenger-google-response";
        var properties = new AuthenticationProperties { RedirectUri = redirectUrl };
        return Challenge(properties, GoogleDefaults.AuthenticationScheme);
    }

    [HttpGet("users/register-passenger-google-response")]
    public async Task<ApiResponse<RegisterUserGoogleResponse>> HandleRegisterPassengerWithGoogle(CancellationToken cancellationToken)
    {
        ...
        var googleRegisterResult = await _mediator.Send(new RegisterUserWithGoogleOrchestrator(RegisterType.Passenger), cancellationToken);
